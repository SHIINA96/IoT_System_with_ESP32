    ...
    if (conn.connect(server_addr, 3306, user, password))
        {
            ...
            // Initiate the query class instance
            MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn);
            sprintf(query_FAN_Manual, INSERT_DATA_FAN_Manual, "FAN", state, "Manual");
            // Execute the query
            cur_mem->execute(query_FAN_Manual);
        }
    else
    Serial.println("Connection failed.");
    conn.close();
    ...
    if (conn.connect(server_addr, 3306, user, password))
        {
            ...
            // Initiate the query class instance
            MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn);
            sprintf(query_PUMP_Manual, INSERT_DATA_PUMP_Manual, "PUMP", state, "Manual");
            // Execute the query
            cur_mem->execute(query_PUMP_Manual);
        }
    else
    Serial.println("Connection failed.");
    conn.close();
